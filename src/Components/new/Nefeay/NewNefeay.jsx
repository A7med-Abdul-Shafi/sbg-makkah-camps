...
const FILE_SIZE = File.size / 1024 / 1024;
  ...
  useEffect(() => {
    const projects = async () => {
      await axios.get(`${api.defaults.baseURL}/nefeay/projects`).then((res) => {
        setOptions(res.data);
      });
    };
    projects();
  ...
  useEffect(() => {
    const projects = async () => {
      await axios.get(`${api.defaults.baseURL}/nefeay/in_reason`).then((res) => {
  ...
  const addResults = async (e) => {
    ...
    setIsLoading(true);
    await axios
      .post(`${api.defaults.baseURL}/nefeay`, addedData)
      .then((response) => {
        Swal.fire({
          position: "center",
          icon: "success",
  ...
  const onSubmit = (data, onSubmitProps) => {
    setIsLoading(true);
    axios
      .post(`${api.defaults.baseURL}/nefeay`, data)
      .then((response) => {
  ...
  useEffect(() => {
    const vacantList = async () => {
      setIsLoading(true);
      await axios.get(`${api.defaults.baseURL}/nefeay/vacant`).then(async (res) => {
        setVacant(res.data);
        setIsLoading(false);
      });
    };
  ...
  useEffect(() => {
    const filterList = async () => {
      setIsLoading(true);
      await axios.get(`${api.defaults.baseURL}/nefeay/filter`).then(async (res) => {
  ...
  useEffect(() => {
    const alameiaRoomList = async () => {
      setIsLoading(true);
      await axios.get(`${api.defaults.baseURL}/nefeayrooms/list`).then(async (res) => {
