...
const FILE_SIZE = File.size / 1024 / 1024;
  ...
  useEffect(() => {
    const projects = async () => {
      await axios.get(`${api.defaults.baseURL}/elfateh/projects`).then((res) => {
        setOptions(res.data);
      });
    };
    projects();
  ...
  useEffect(() => {
    const projects = async () => {
      await axios.get(`${api.defaults.baseURL}/elfateh/in_reason`).then((res) => {
  ...
  const addResults = async (e) => {
    ...
    setIsLoading(true);
    await axios
      .post(`${api.defaults.baseURL}/elfateh`, addedData)
      .then((response) => {
        Swal.fire({
          position: "center",
          icon: "success",
  ...
  const onSubmit = (data, onSubmitProps) => {
    setIsLoading(true);
    axios
      .post(`${api.defaults.baseURL}/elfateh`, data)
      .then((response) => {
  ...
  useEffect(() => {
    const vacantList = async () => {
      setIsLoading(true);
      await axios.get(`${api.defaults.baseURL}/elfateh/vacant`).then(async (res) => {
        setVacant(res.data);
        setIsLoading(false);
      });
    };
  ...
  useEffect(() => {
    const filterList = async () => {
      setIsLoading(true);
      await axios.get(`${api.defaults.baseURL}/elfateh/filter`).then(async (res) => {
  ...
  useEffect(() => {
    const alameiaRoomList = async () => {
      setIsLoading(true);
      await axios.get(`${api.defaults.baseURL}/elfatehrooms/list`).then(async (res) => {
